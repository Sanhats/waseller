...
- Con **`LLM_SHADOW_COMPARE_URL`** y sin opt-out, Waseller **delega por defecto** el NL de leads al crew (mismo pipeline que `WASELLER_CREW_DELEGATE_CONVERSATION` implícito en `true`). Opt-out explícito: `WASELLER_CREW_DELEGATE_CONVERSATION=false`.
- **`WASELLER_CREW_MANDATORY=true`** (opcional): con URL, **siempre** delegación NL al crew; **anula** `WASELLER_CREW_DELEGATE_CONVERSATION=false`. Útil cuando en este repo solo querés armar contexto + POST y la lógica conversacional vive en waseller-crew.
