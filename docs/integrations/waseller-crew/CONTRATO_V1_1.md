...
**Documentación cruzada:** en el repo **waseller-crew** el archivo `docs/CONTRATO_HTTP_V1_1.md` describe el mismo contrato (tabla ampliada, checklist §5, fixture `fixtures/request.mesa_colores.json`, test `test_mesa_colores_fixture_parses_and_posts`) y **remite a este documento** como referencia de lo que serializa Waseller (main). **Paridad de campos opcionales:** los nombres y límites de `etapa`, `activeOffer`, `memoryFacts` (raíz), `publicCatalogSlug` / `publicCatalogBaseUrl`, `tenantRuntimeContext` y el uso de `tenantBrief` coinciden con el `ShadowCompareRequest` del crew (este último objeto con `extra="ignore"` en claves aún no modeladas); Waseller también enriquece `interpretation` con digests (§1.1) para compatibilidad y menor token count donde aplica.
...
**Orquestador primero (`WASELLER_CREW_ORCHESTRATE_FIRST=true`):** con LLM habilitado, `LLM_SHADOW_COMPARE_URL` y **`WASELLER_CREW_PRIMARY` o `WASELLER_CREW_SOLE_MODE`**, el **message-processor** puede enrutar al **orquestador** turnos que antes iban solo al **lead** (p. ej. variante ya resuelta), para que el crew reciba RAG + `stockTable` enriquecido (en sole, stub + crew en orquestador). Cobro / reserva explícita puede seguir forzando lead; ver código en `message-processor.worker.ts`.
...
| `tenantRuntimeContext` | `object` | Opcional | Snapshot **ordenado** de la fila `tenants` + metadatos de `tenant_knowledge` (categoría/etiquetas/fecha) + políticas LLM del tenant + tiempos de envío masivo + integraciones de pago (**solo** `provider` y `status`, sin secretos). Incluye `catalog.publicSlug` / `catalog.publicBaseUrl` cuando aplica (misma regla que los campos raíz de catálogo). **No** duplica el JSON completo de `tenant_knowledge.profile`: el detalle comercial sigue yendo en `tenantBrief` / `tenantCommercialContext` cuando el worker lo arma. Tipado Waseller: `CrewTenantRuntimeContextV1` en `shadow-compare.service.ts`. Desactivar envío: `LLM_SHADOW_COMPARE_OMIT_TENANT_RUNTIME_CONTEXT=true`. Número WhatsApp del canal bot del tenant: **no** se envía salvo `LLM_SHADOW_COMPARE_INCLUDE_TENANT_WHATSAPP_NUMBER=true`. |
...
| `LLM_SHADOW_COMPARE_OMIT_TENANT_RUNTIME_CONTEXT` | Opcional | `true`/`1`/`yes`: **no** incluye el objeto `tenantRuntimeContext` en el body (útil para acortar payload o entornos de prueba). |
| `LLM_SHADOW_COMPARE_INCLUDE_TENANT_WHATSAPP_NUMBER` | Opcional | `true`/`1`/`yes`: dentro de `tenantRuntimeContext.channel` envía `whatsAppBusinessNumber` (número del bot del tenant). Por defecto **no** se envía (privacidad). |
| `LLM_SHADOW_COMPARE_MAX_TENANT_RUNTIME_DISPLAY_NAME_CHARS` | Opcional | Tope de `tenantRuntimeContext.identity.displayName` (default **160**). |
| `LLM_SHADOW_COMPARE_MAX_TENANT_RUNTIME_LABELS` | Opcional | Máximo de etiquetas en `tenantRuntimeContext.knowledge.businessLabels` (default **24**). |
| `LLM_SHADOW_COMPARE_MAX_TENANT_RUNTIME_PAYMENT_CHANNELS` | Opcional | Máximo de filas en `tenantRuntimeContext.paymentChannels` (default **12**). |
| `LLM_SHADOW_COMPARE_MAX_TENANT_RUNTIME_WHATSAPP_CHARS` | Opcional | Tope de `whatsAppBusinessNumber` si se incluye (default **48**). |
| `LLM_SHADOW_COMPARE_MAX_TENANT_RUNTIME_MODEL_NAME_CHARS` | Opcional | Tope de `tenantRuntimeContext.llm.modelName` (default **80**). |
...
| `WASELLER_CREW_SOLE_MODE` | Opcional | `true`/`1`/`yes`: orquestador sin intérprete OpenAI ni decide interno; **lead worker** directo envía stub al crew (ver `README.md`). Habilita POST al crew sin exigir `WASELLER_CREW_PRIMARY`. |
| `WASELLER_CREW_ORCHESTRATE_FIRST` | Opcional | `true`/`1`/`yes`: con LLM habilitado y URL + (**primary** o **sole**), el **message-processor** enruta también turnos con variante resuelta al **orquestador** primero. Ver `README.md` y `message-processor.worker.ts`. |
