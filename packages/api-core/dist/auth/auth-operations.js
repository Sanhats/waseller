...
exports.authRuntimeEnvFromProcess = void 0;
...
exports.authRuntimeEnvFromProcess = authRuntimeEnvFromProcess;
const hashPassword = (pepper, tenantId, password) => (0, node_crypto_1.createHash)("sha256").update(`${password}:${tenantId}:${pepper}`).digest("hex");
const issueToken = (runtime, input) => {
    const { token, exp } = (0, token_1.createAuthToken)(runtime, {
        sub: input.userId,
...
async function loginWithTenant(prisma, runtime, tenantId, email, password) {
...
async function loginUser(prisma, runtime, input) {
    ...
    if (users.length > 1) {
        return {
            ...
            error: {
                status: 409,
                message: "Tu email está asociado a más de un tenant. Contactá soporte para habilitar selección de workspace."
            }
        };
    }
    const user = users[0];
...
async function registerTenantUser(prisma, runtime, input) {
    ...
    if (!tenantName)
        return { ok: false, error: { status: 400, message: "tenantName es requerido" } };
    if (!whatsappNumber)
        return { ok: false, error: { status: 400, message: "whatsappNumber es requerido" } };
    ...
    if (!email)
        return { ok: false, error: { status: 400, message: "email es requerido" } };
    if (!emailRegex.test(email))
        return { ok: false, error: { status: 400, message: "Formato de email invalido" } };
    ...
    if (whatsappTaken) {
        return {
            ...
            error: {
                status: 409,
                message: "Ese número de WhatsApp ya está asociado a otro negocio. Si es tuyo, iniciá sesión; si necesitás otra cuenta, usá otro número."
            }
        };
    }
    ...
    if (emailTaken) {
        return {
            ...
            error: {
                status: 409,
                message: "Ese correo electrónico ya está registrado. Iniciá sesión o usá otro email para crear un negocio nuevo."
            }
        };
    }
    let tenant;
