...
async function fetchOtherInStockProductNames(tenantId, excludeProductId, limit) {
...
function extractMentionedQuantityUnits(text) {
    const t = normalizeText(text);
    const strong = t.match(/\b(?:necesito|quiero|pido|pedimos|compro|estoy\s+necesitando)\s*(?:al\s*menos\s*)?(\d{1,4})\b/);
    if (strong) {
...
function isCatalogBroadDiscoveryAsk(raw) {
    const t = normalizeText(raw);
    return (/\b(que\s+otro\s+producto|qu[eé]\s+otro\s+producto|otro\s+producto|otros\s+productos|otra\s+cosa)\b/.test(t) ||
        /\b(disponible\s+en\s+(el\s+)?catalogo|en\s+el\s+catalogo|todo\s+el\s+catalogo|que\s+hay\s+en\s+(el\s+)?catalogo)\b/.test(t) ||
        /\b(que\s+mas\s+tenes|qu[eé]\s*m[aá]s\s+tenes|que\s+tienen\s+disponible|que\s+más\s+hay)\b/.test(t));
}
function isAnotherProductOrLineAsk(raw) {
    const t = normalizeText(raw);
    if (/\b(otra\s+mesa|otro\s+modelo|otra\s+linea|otro\s+articulo|otro\s+item)\b/.test(t))
        return true;
    if (/\bno\s*tenes\s*otra\b/.test(t) && /\b(mesa|silla|escritorio|producto|opcion)\b/.test(t))
        return true;
    if (/\botro[s]?\s+(tipo|modelo|estilo)\b/.test(t))
        return true;
    return false;
}
function filterOthersByLooseTokens(raw, others) {
    const t = normalizeText(raw);
    const stop = new Set([
        "tenes",
        "tienen",
...
function wantsVariantAxisFollowUp(raw) {
    const plain = normalizeText(raw);
    if (isCatalogBroadDiscoveryAsk(raw) || isAnotherProductOrLineAsk(raw))
        return false;
    if (/\b(otro\s+producto|otros\s+productos)\b/.test(plain))
        return false;
    if (/\b(no\s+me\s+alcanza|no\s+alcanza|necesito\s+m[aá]s|m[aá]s\s+unidades|no\s+tenes\s+m[aá]s|no\s+tienen\s+m[aá]s)\b/.test(plain))
        return false;
    if (/\b(otro\s+color|otra\s+vari|otro\s+talle|otra\s+talla|otra\s+combinaci|otra\s+opc|tenes\s+en\s+otro|tenes\s+en\s+otra|hay\s+en\s+otro)\b/.test(plain))
        return true;
    if (/\b(otro|otra)\b/.test(plain) && /\b(color|talle|talla|vari)\b/.test(plain))
        return true;
    return false;
}
const AXIS_LABELS = {
...
exports.leadWorker = new bullmq_1.Worker(src_1.QueueNames.leadProcessing, async (job) => {
    ...
    const catalogOrAlternateProduct = variant &&
    ...
    const shortageFrustration = /\b(no\s+me\s+alcanza|no\s+alcanza|necesito\s+m[aá]s|m[aá]s\s+unidades|no\s+tenes\s+m[aá]s|no\s+tienen\s+m[aá]s)\b/.test(normalizeText(rawIncoming));
