...
exports.SelfHostedLlmService = void 0;
const bot_template_service_1 = require("./bot-template.service");
const clamp = (value, min, max) => Math.max(min, Math.min(max, value));
class SelfHostedLlmService {
    endpoint = process.env.LLM_SELF_HOSTED_URL;
    model = process.env.LLM_MODEL_NAME ?? "self-hosted-default";
    openAiModel = process.env.OPENAI_MODEL ?? "gpt-4o-mini";
    openAiApiKey = process.env.OPENAI_API_KEY;
    timeoutMs = Number(process.env.LLM_TIMEOUT_MS ?? 8000);
    templateService = new bot_template_service_1.BotTemplateService();
    parseJsonReply(raw) {
    ...
    async fallback(context) {
        ...
        const productName = (typeof context.interpretation?.entities?.productName === "string"
            ? context.interpretation.entities.productName
            : null) ??
            activeOfferProduct ??
            product?.name ??
            null;
        const reportedPaid = /ya pague|ya pague|pago realizado|ya transferi|te transferi|comprobante/.test(normalizedInput);
        const wantsPrice = /precio|cu[aá]nto|sale/i.test(context.incomingText);
        const asksPaymentOptions = /como puedo pagar|formas? de pago|medios? de pago|aceptan|se puede pagar|pago en efectivo|pagar en efectivo/.test(normalizedInput);
        const wantsDirectPaymentLink = /(pasame|enviame|mandame).*(link|alias)|quiero.*link|compart(i|í).*(link|alias)/.test(normalizedInput);
        ...
        const nextAction = interpretedNextAction
    ...
    async decideWithOpenAI(context) {
...
exports.SelfHostedLlmService = SelfHostedLlmService;
