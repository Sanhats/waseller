...
exports.LlmVerifierService = void 0;
const clamp = (value) => Math.max(0, Math.min(1, value));
class LlmVerifierService {
    endpoint = process.env.LLM_VERIFIER_URL;
    model = process.env.LLM_VERIFIER_MODEL ?? process.env.LLM_MODEL_NAME ?? "verifier-default";
    timeoutMs = Number(process.env.LLM_VERIFIER_TIMEOUT_MS ?? 4000);
    rulesVerify(input) {
        const flags = [];
        ...
        if (normalizedReply === incoming)
            flags.push("echo_reply");
        if (/^(si+|sii+|dale|ok)[\s,.-]*(enviame|mandame|pasame)\b/i.test(reply))
            flags.push("role_confusion");
        if (/100%\s*seguro|garantizado/i.test(reply))
            flags.push("overpromise");
        const tenantPayment = input.tenantProfile?.payment?.methods ?? [];
        ...
        const tenantShipping = input.tenantProfile?.shipping?.methods ?? [];
...
exports.LlmVerifierService = LlmVerifierService;
