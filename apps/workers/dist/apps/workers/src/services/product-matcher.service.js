...
exports.ProductMatcherService = void 0;
...
class ProductMatcherService {
    ...
    buildProductAxisValues(variants, requiredAxes) {
        ...
        for (const item of variants) {
            ...
            for (const axis of requiredAxes) {
                ...
                if (!value)
                    continue;
                const bucket = current[axis] ?? new Set();
                bucket.add(value);
                current[axis] = bucket;
            }
            byProduct.set(item.productId, current);
        }
        return new Map(Array.from(byProduct.entries()).map(([productId, axisMap]) => [
            productId,
            Object.fromEntries(Object.entries(axisMap).map(([axis, values]) => [axis, Array.from(values)]))
        ]));
    }
    async matchByMessage(tenantId, message, context) {
        ...
        const requiredAxes = Array.isArray(context?.requiredAxes)
            ...
            : [];
        const variants = (await src_1.prisma.$queryRaw `
      select
        p.id as "productId",
    ...
    `);
        const normalized = variants.map((item) => ({
            ...
            sku: item.sku,
            attributes: Object.fromEntries(Object.entries(item.attributes ?? {}).map(([key, value]) => [
                this.normalize(key),
        ...
        const resolveRequestedAttributes = (productId) => {
            const axisValues = productAxisValues.get(productId) ?? {};
            return Object.fromEntries(requiredAxes.flatMap((axis) => {
        ...
        const resolveMissingAxes = (productId, requestedAttributes) => {
            const axisValues = productAxisValues.get(productId) ?? {};
        ...
        const buildMatchResult = (selectedProduct) => {
            const requestedAttributeCount = Object.keys(selectedProduct.requestedAttributes).length;
            const exactVariants = selectedProduct.productVariants.filter((item) => Object.entries(selectedProduct.requestedAttributes).every(([axis, value]) => item.attributes[axis] === value));
            const exact = exactVariants[0];
            const onlyOnePossibleVariant = selectedProduct.productVariants.length === 1 || (requestedAttributeCount > 0 && exactVariants.length === 1);
        ...
        const contextualProduct = productScores.find((item) => this.normalize(item.productName) === previousProductNormalized);
        const isEllipticContinuation = /\b(si|dale|ok|listo|compr(o|a)|reserva|apart(a|ame)|pago|efectivo|puede\s+ser)\b/.test(text);
        const hasContextualAttributes = Object.keys(contextualProduct?.requestedAttributes ?? {}).length > 0;
        const shouldPreferContext = Boolean(contextualProduct) &&
        ...
        if (!bestProduct || bestProduct.finalScore <= 0 || text.trim().length === 0)
            return null;
        return {
...
exports.ProductMatcherService = ProductMatcherService;
