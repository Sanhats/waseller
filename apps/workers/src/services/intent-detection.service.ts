...
export class IntentDetectionService {
  ...
  detect(message: string): Intent {
    ...
    const isMostlyThanks =
      this.hasAny(text, ["gracias", "muchas gracias", "mil gracias"]) &&
      !this.hasAny(text, ["precio", "stock", "link", "pago", "compr", "reserv", "talle", "envio"]);
    const hasStrongBuySignal =
      this.hasAny(text, ["quiero", "comprar", "compro", "confirmo", "reserv", "apartar"]) &&
      this.hasAny(text, ["pasame", "alias", "link", "pago", "transfer", "efectivo"]);
    const asksHuman =
      this.hasAny(text, ["asesor", "humano", "persona", "vendedor", "hablar con alguien", "atencion humana"]) ||
      /hablar con (alguien|una persona|un asesor)/.test(text);
    const asksPaymentLink =
      /(pasame|enviame|mandame|compartime|compartime).*(link|checkout)/.test(text) ||
      this.hasAny(text, ["link de pago", "mandame link", "enviame link", "pasame el link"]);
    const asksAlternative =
      this.hasAny(text, [
        "que otra",
        "qué otra",
        "otra opcion",
    ...
    const rejectsOffer =
      this.hasAny(text, ["esa no", "no esa", "no me sirve", "no me va", "no me gusta", "prefiero otra"]) ||
      /^(no|nop|ni ahi|ni ah[ií])[.!? ]*$/.test(shortText);
    const acceptsOffer =
      /^(si|sí|si dale|sí dale|dale|de una|ok|oka|okey|perfecto|joya|me sirve|me va|voy con esa|esa quiero|la quiero|lo quiero)[.!? ]*$/.test(
        shortText
      ) ||
      this.hasAny(text, ["dejame una", "dejame uno", "esa quiero", "voy con esa", "me sirve"]);
    const choosesVariantByReference =
      this.hasAny(text, ["la blanca", "la negra", "la roja", "la azul", "la de 39", "la de 40", "la otra", "esa"]) ||
      ((text.includes("talle") || text.includes("color")) && words.length <= 8);
    const asksShipping =
      this.hasAny(text, ["envio", "envío", "hacen envios", "hacen envíos", "mandan", "entrega", "correo", "moto"]);
    const asksPickup =
      this.hasAny(text, ["retiro", "retirar", "paso a buscar", "se retira", "retiro por", "paso manana", "paso mañana"]);
    const choosesPaymentMethod =
      this.hasAny(text, ["efectivo", "transferencia", "alias", "tarjeta", "debito", "débito", "credito", "crédito"]) &&
      !asksPaymentLink;

    if (isMostlyThanks) return "saludo";
    if (asksHuman) return "pedir_asesor";
    ...
    if (asksPaymentLink) return "pedir_link_pago";
    if (choosesPaymentMethod) return "elegir_medio_pago";
    if (asksShipping) return "preguntar_envio";
    if (asksPickup) return "preguntar_retiro";
    ...
    if (text.includes("cancel")) return "cancelar";

    const pricePhrases = ["precio", "cuesta", "cuanto", "vale", "valor"];
    if (this.hasAny(text, pricePhrases)) return "consultar_precio";
    ...
    if (text.includes("hola") || text.includes("buenas")) return "saludo";
    if (text.includes(",") || (text.includes(" y ") && words.length >= 4)) return "multi_producto";
    if (text.includes("sin stock") || text.includes("agotado")) return "sin_stock";
    return "desconocida";
  }

  isBusinessRelated(message: string, hasMatchedProduct = false): boolean {
    if (hasMatchedProduct) return true;
    const text = this.normalize(message);
