...
function googleFontsUrl(cfg: ReturnType<typeof normalizeStoreConfig>): string | null {
...
export default async function TiendaSlugLayout({ params, children }: LayoutProps) {
  ...
  return (
      ...
      style={
        {
          ...
          "--ts-editorial-muted": "color-mix(in srgb, var(--ts-border) 50%, var(--ts-bg))",
          "--ts-media-scrim": "color-mix(in srgb, var(--ts-text) 30%, transparent)",
          "--ts-overlay-backdrop": "color-mix(in srgb, var(--ts-text) 42%, transparent)",
          backgroundColor: BG,
          color: TEXT,
    ...
    >
      ...
      <TiendaNavbar
        ...
        topBarCenter={cfg.brand.tagline}
        currencyLabel={cfg.uiTexts.currencySymbol || "ARS"}
      />
      ...
      <footer className="mt-16 border-t" style={{ borderColor: "var(--ts-border)", backgroundColor: "var(--ts-surface)" }}>
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14">
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-16">
            <div>
              ...
              <p className="mt-2 max-w-md text-sm leading-relaxed" style={{ color: "var(--ts-muted)" }}>
                Dejanos tu email y te avisamos cuando haya nuevos productos o promociones.
              </p>
              ...
              <p className="mt-2 text-[11px]" style={{ color: "var(--ts-muted)" }}>
                Próximamente podrás activar el envío desde el panel de la tienda.
              </p>
            </div>

            <div className="grid grid-cols-1 gap-10 sm:grid-cols-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.14em]" style={{ color: "var(--ts-text)" }}>
                  Ayuda
                </p>
                <ul className="mt-4 space-y-2 text-sm" style={{ color: "var(--ts-muted)" }}>
                  {cfg.contact.email && (
                    <li>
                      <a href={`mailto:${cfg.contact.email}`} className="no-underline hover:underline" style={{ color: "var(--ts-muted)" }}>
                        {cfg.contact.email}
                      </a>
                    </li>
                  )}
                  {cfg.contact.phone && (
                    <li>
                      <a href={`tel:${cfg.contact.phone}`} className="no-underline hover:underline" style={{ color: "var(--ts-muted)" }}>
                        {cfg.contact.phone}
                      </a>
                    </li>
                  )}
                  {!cfg.contact.email && !cfg.contact.phone && (
                    <li className="text-xs">Contacto por WhatsApp al comprar.</li>
                  )}
                </ul>
              </div>
              <div>
                ...
                <ul className="mt-4 space-y-2 text-sm">
                  <li>
                    <Link href={`/tienda/${slug}`} className="no-underline hover:underline" style={{ color: "var(--ts-muted)" }}>
                      Inicio
                    </Link>
                  </li>
                  <li>
                    <Link href={`/tienda/${slug}/catalogo`} className="no-underline hover:underline" style={{ color: "var(--ts-muted)" }}>
                      Catálogo
                    </Link>
                  </li>
                </ul>
              ...
              <div>
                ...
                <ul className="mt-4 space-y-2 text-sm" style={{ color: "var(--ts-muted)" }}>
                  <li className="text-xs leading-relaxed">
                    Catálogo público. Precios y stock pueden variar; confirmá antes de abonar.
                  </li>
                </ul>
              </div>
            </div>
          ...
          {(cfg.contact.instagram || cfg.contact.facebook || cfg.contact.tiktok || cfg.contact.pinterest) && (
            <div className="mt-10 border-t pt-8" style={{ borderColor: "var(--ts-border)" }}>
              <p className="mb-3 text-xs font-bold uppercase tracking-[0.14em]" style={{ color: "var(--ts-text)" }}>
                Redes
              </p>
        ...
        <div className="py-3 text-center" style={{ backgroundColor: "var(--ts-primary)" }}>
          <p
            className="px-4 text-[10px]"
            style={{ color: "color-mix(in srgb, var(--ts-on-primary) 72%, transparent)" }}
          >
            © {new Date().getFullYear()} {storeName} · Catálogo público
          </p>
        </div>
      </footer>
    </div>
