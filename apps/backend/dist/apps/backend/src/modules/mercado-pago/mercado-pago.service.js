...
let MercadoPagoService = MercadoPagoService_1 = class MercadoPagoService {
    ...
    ensureConfigured() {
        if (!this.clientId || !this.clientSecret || !this.redirectUri) {
            throw new common_1.BadRequestException("Falta configurar MERCADO_PAGO_CLIENT_ID, MERCADO_PAGO_CLIENT_SECRET, y MERCADO_PAGO_REDIRECT_URI o PUBLIC_API_BASE_URL (se usa .../api/integrations/mercadopago/callback).");
        }
    }
    buildPopupHtml(options) {
    ...
    async loadIntegrationByTenant(tenantId) {
    ...
    async loadIntegrationByExternalAccount(externalAccountId) {
        const row = await src_1.prisma.tenantPaymentIntegration.findFirst({
    ...
    async upsertIntegration(tenantId, payload) {
        const accessTokenEncrypted = payload.accessToken ? (0, src_3.encryptIntegrationSecret)(payload.accessToken) : null;
        const refreshTokenEncrypted = payload.refreshToken ? (0, src_3.encryptIntegrationSecret)(payload.refreshToken) : null;
     ...
     */
    async persistIntegrationBestEffort(tenantId, payload) {
        try {
            await this.upsertIntegration(tenantId, payload);
            return true;
        }
        catch (error) {
            this.logger.error(`No se pudo persistir tenant_payment_integrations (tenant=${tenantId}): ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    async requestOauthToken(body) {
        ...
        if (!response.ok) {
            ...
            throw new common_1.InternalServerErrorException(rawText || `Mercado Pago oauth/token responded with ${response.status}`);
        }
        try {
            ...
            throw new common_1.InternalServerErrorException("Mercado Pago oauth/token devolvió una respuesta inválida.");
        }
    }
    async refreshIfNeeded(row) {
        const expiresAt = row.expiresAt ? new Date(row.expiresAt) : null;
        const accessToken = row.accessTokenEncrypted ? (0, src_3.decryptIntegrationSecret)(row.accessTokenEncrypted) : "";
        ...
        if (!row.refreshTokenEncrypted) {
            ...
            throw new common_1.BadRequestException("La conexión de Mercado Pago venció. Reconectá la cuenta.");
        }
        const refreshed = await this.requestOauthToken({
            client_id: this.clientId,
            client_secret: this.clientSecret,
            grant_type: "refresh_token",
            refresh_token: (0, src_3.decryptIntegrationSecret)(row.refreshTokenEncrypted)
        });
        ...
        await this.upsertIntegration(row.tenantId, {
            ...
            refreshToken: refreshed.refresh_token ?? (0, src_3.decryptIntegrationSecret)(row.refreshTokenEncrypted),
    ...
    async getConnectUrl(tenantId) {
        ...
        const state = (0, src_3.signMercadoPagoState)({
            tenantId,
    ...
    async handleCallback(query) {
        try {
            this.ensureConfigured();
            const signedState = (0, src_3.verifyMercadoPagoState)(String(query.state ?? ""));
            ...
            try {
                ...
                if (!saved) {
                    return this.buildPopupHtml({
                        ok: false,
                        tenantId: signedState.tenantId,
                        message: "Mercado Pago autorizó la app, pero no pudimos guardar las credenciales en el servidor. Revisá la base de datos o los logs."
                    });
                }
                return this.buildPopupHtml({
                    ok: true,
                ...
                const lastError = error instanceof Error ? error.message : "No se pudo completar OAuth.";
                await this.persistIntegrationBestEffort(signedState.tenantId, {
                    status: "error",
                    externalAccountId: null,
                    externalAccountLabel: null,
            ...
            this.logger.error(`handleCallback inesperado: ${error instanceof Error ? error.stack ?? error.message : String(error)}`);
            return this.buildPopupHtml({
                ok: false,
    ...
    async notifyPaymentStatus(input) {
        ...
        if (!phone)
            return;
        const dedupeKey = (0, src_2.buildStableDedupeKey)("mercadopago-webhook", input.tenantId, input.leadId, input.paymentId, input.status);
        await src_2.outgoingQueue.add("payment-status-auto-v1", {
    ...
    async handleWebhook(input) {
        ...
        if (this.webhookSecret) {
            ...
            const valid = (0, src_3.verifyMercadoPagoWebhookSignature)({
    ...
    `;
        ...
        if (previousStatus !== mappedStatus) {
            const paymentMetadata = typeof payment.metadata === "object" && payment.metadata !== null
                ? payment.metadata
                : {};
            const attemptMetadata = typeof attempt.metadata === "object" && attempt.metadata !== null
                ? attempt.metadata
                : {};
            const productName = String(paymentMetadata.product_name ?? attemptMetadata.productName ?? attempt.title ?? "tu compra").trim();
    ...
    get checkoutPreferenceWebhookUrl() {
    ...
    async createDraftCheckoutPreference(input) {
        ...
        const createdAttempt = await src_1.prisma.paymentAttempt.create({
            data: {
                ...
                provider: "mercadopago",
                status: "draft",
                amount: new library_1.Decimal(Number(input.amount)),
                currency: "ARS",
                title: input.title,
                externalReference,
